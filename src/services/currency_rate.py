...
from src.validation_rules.currency_rate.create import CreateCurrencyRateData
...
class CurrencyRateService:
    ...
    @allow_roles(["admin"])
    def create(self, data: CreateCurrencyRateData, *args, **kwargs):
    ...
    def delete(self, id_: int, *args, **kwargs):
        ...
        if self._order_repo.has_for_date_range(
            rate_to_delete.created_on, next_rate.created_on if next_rate else None
        ):
    ...
    @allow_roles(["admin"])
    def get_one(self, id_: int, *args, **kwargs):
        try:
